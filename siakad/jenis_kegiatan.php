...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        ...
        switch ($action) {
            case 'create':
                $result = $jenisKegiatanModel->create($data);
                $message = $result !== false ? 'Jenis kegiatan berhasil ditambahkan' : 'Gagal menambahkan jenis kegiatan';
                break;
            case 'edit':
                $id = (int) $_POST['id'];
                $result = $jenisKegiatanModel->update($id, $data);
                $message = $result ? 'Jenis kegiatan berhasil diperbarui' : 'Gagal memperbarui jenis kegiatan';
                break;
            case 'delete':
                $id = (int) $_POST['id'];
                $result = $jenisKegiatanModel->delete($id);
                $message = $result ? 'Jenis kegiatan berhasil dihapus' : 'Gagal menghapus jenis kegiatan';
                break;
        }
...
<div x-data="{ showModal: false, modalMode: 'create', formData: {} }">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Jenis Kegiatan</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">Manajemen data jenis kegiatan akademik</p>
        </div>
    ...
    <div
        ...
        <div class="p-6 overflow-x-auto">
            <table id="jenisKegiatanTable" class="min-w-full table-auto">
                <thead class="bg-gray-50 dark:bg-gray-700/30">
                    <tr>
                        <th
                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                            Nama Jenis Kegiatan</th>
                        <th
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right">
                                    <div class="flex justify-end space-x-2">
                                        <button type="button"
                                            @click="showModal = true; modalMode = 'edit'; formData = <?= htmlspecialchars(json_encode($jk)) ?>"
                                        ...
                                        <button type="button"
                                            @click="showModal = true; modalMode = 'delete'; formData = <?= htmlspecialchars(json_encode($jk)) ?>"
                        ...
                        <tr>
                            <td colspan="3" class="px-6 py-10 text-center text-gray-500 dark:text-gray-400">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p>Tidak ada data jenis kegiatan yang tersedia.</p>
                                    <button type="button" @click="showModal = true; modalMode = 'create'; formData = {}"
                ...
                <form method="POST">
                                ...
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    ...
                                    <div class="mt-2">
                                        <p class="text-sm text-gray-500 dark:text-gray-400"
                                            x-text="`Apakah Anda yakin ingin menghapus jenis kegiatan '${formData.nama}'? Tindakan ini tidak dapat dibatalkan.`">
                                        </p>
                                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">
                                            Perhatian: Menghapus jenis kegiatan akan memengaruhi semua kegiatan yang
                                            terkait.
                                        </p>
                                    </div>
                                </div>
                    ...
                    <template x-if="modalMode !== 'delete'">
                        <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="mb-4">
                                <h3 class="text-lg font-medium leading-6 text-gray-900 dark:text-white"
                                    x-text="modalMode === 'edit' ? 'Edit Jenis Kegiatan' : 'Tambah Jenis Kegiatan'">
                                </h3>
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400"
                                    x-text="modalMode === 'edit' ? 'Perbarui informasi jenis kegiatan.' : 'Isi form berikut untuk menambahkan jenis kegiatan baru.'">
                                </p>
                            </div>
...
$pageVars = [
    'title' => 'Jenis Kegiatan - Sistem Manajemen Kegiatan Dosen',
    'description' => 'Manajemen data jenis kegiatan akademik',
    'pageTitle' => 'Jenis Kegiatan',
